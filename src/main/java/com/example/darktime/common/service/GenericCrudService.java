...
import com.example.darktime.common.rsql.SpecificationBuilder;
...
public abstract class GenericCrudService<ENTITY, ID, CREATE_DTO, UPDATE_DTO, RESPONSE_DTO> {
    ...
    protected abstract GenericDtoMapper<ENTITY, ID, CREATE_DTO, UPDATE_DTO, RESPONSE_DTO> getMapper();
    ...
    public Page<RESPONSE_DTO> getAll(Pageable pageable, String predicate) {
        ...
        if (specification == null) {
            return getRepository().findAll(pageable).map(entity -> getMapper().toResponseDto(entity));
        }

        return getRepository().findAll(specification, pageable).map(entity -> getMapper().toResponseDto(entity));

    }

    public RESPONSE_DTO getById(ID id) {
        // todo exception handle qil
